...
function Sculpture({state,reduced,onReady}:{state:string,reduced:boolean,onReady:()=>void}){
 const {scene}=useGLTF(import.meta.env.BASE_URL+'models/sgs-core.glb',false,false);useEffect(onReady,[onReady]);const clone=useMemo(()=>scene.clone(true),[scene]);const ref=useRef<Group>(null);const modules=useMemo(()=>{const a:{mesh:Mesh,base:Vector3}[]=[];clone.traverse(o=>{if(o.name.startsWith('Module'))a.push({mesh:o as Mesh,base:o.position.clone().normalize().multiplyScalar(Math.sqrt(3)*.56)});});return a;},[clone]);
...
export function Core({state,dark,reduced}:{state:string,dark:boolean,reduced:boolean}){const [loaded,setLoaded]=useState(false);const onReady=useCallback(()=>setLoaded(true),[]);const [visible,setVisible]=useState(!document.hidden);useEffect(()=>{const f=()=>setVisible(!document.hidden);document.addEventListener('visibilitychange',f);return()=>document.removeEventListener('visibilitychange',f);},[]);return <div className="core-canvas" data-loaded={loaded} role="img" aria-label="Синк — модульное ядро цифрового интеллекта"><Boundary><Canvas dpr={[1,1.5]} camera={{position:[0,1.1,6.7],fov:35}} frameloop={visible?(reduced?'demand':'always'):'never'} onCreated={s=>{if(import.meta.env.DEV)(window as any).__sgsThree=s;}} gl={{alpha:true,antialias:true,powerPreference:'low-power'}}><ambientLight intensity={dark?1.3:1.9}/><directionalLight position={[4,6,4]} intensity={4}/><directionalLight position={[-4,2,-1]} color="#5277ef" intensity={dark?5:2}/><pointLight position={[0,0,3]} intensity={8}/><Suspense fallback={null}><Sculpture state={state} reduced={reduced} onReady={onReady}/><ContactShadows frames={1} position={[0,-1.6,0]} opacity={dark?.28:.17} scale={8} blur={2.5} far={4} resolution={128}/></Suspense></Canvas></Boundary></div>;}
