...
try{
 ...
 await check('Per-minute request limit is enforced',async()=>{const ids=[randomUUID(),randomUUID(),randomUUID()];for(const x of ids)await db.query("INSERT INTO ai_usage(id,visitor_id,status) VALUES($1,$2,'complete')",[x,visitorId]);try{assert.equal((await req('/chat',{message:'Продажи'})).status,429);}finally{await db.query('DELETE FROM ai_usage WHERE id=ANY($1::uuid[])',[ids]);}});
