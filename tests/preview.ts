...
try{
 await page.goto(url);await page.bringToFront();const c=await context.newCDPSession(page);await c.send('Emulation.setFocusEmulationEnabled',{enabled:true});await page.waitForFunction(()=>document.querySelector('.core-canvas')?.getAttribute('data-loaded')==='true');await page.locator('.composer-footer').filter({hasText:'Демонстрационный стенд'}).waitFor();tests.push('Public preview and GLB loaded');
 ...
 await page.reload();await page.locator('.composer-footer').filter({hasText:'Демонстрационный стенд'}).waitFor();assert.equal(await page.locator('.message').count(),0);tests.push('Chat memory clears on reload');
